...
from schemas import UserInfo , UserBody , SuccessMsg , Csrf
...
async def login(request: Request, response: Response, user: UserBody, csrf_protect: CsrfProtect = Depends()):
    ...
    response.set_cookie(
        # httponlyをtrueにして、javascriptからcookieを読み取れなくする（不正なアクセスを防ぐ）。
        # CSRFtokenを利用して、クライアントからのアクセスなのか、不正なアクセスなのかを判定することができる。
